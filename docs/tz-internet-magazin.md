...
**Постоянная клавиатура (по запросу, не только по пушу):** после `handle_start` бот присылает `ReplyKeyboardMarkup` с кнопками «📦 Заказы» и «📊 Статистика», которая остаётся в чате до явного скрытия. «📦 Заказы» открывает инлайн-фильтры по статусу (Все/Новые/Ожидают оплаты/В сборке/Отменённые/Возвраты), список — постранично по 8 через тот же `list_orders_admin`, что и `GET /admin/orders`, а карточка конкретного заказа (`order_view:{id}`) рендерится той же функцией, что и в пуш-уведомлениях. «📊 Статистика» даёт выбор периода (сегодня/7д/30д) и печатает выручку/число заказов/топ-3 товара через ту же статистику, что и `/admin/stats/summary`. Оба текстовых обработчика и все новые callback-и (`orders_list:*`, `order_view:*`, `stats:*`) проходят ту же проверку привязки chat_id и роли, что и кнопки смены статуса.
